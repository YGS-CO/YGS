local function callback_reply(extra, success, result)
	--icon & rank ------------------------------------------------------------------------------------------------
	userrank = "Member"
	if tonumber(result.from.id) == 350472483 then
		userrank = "Master ⭐⭐⭐⭐"
...
local function callback_res(extra, success, result)
	...
	--icon & rank ------------------------------------------------------------------------------------------------
	if tonumber(result.id) == 350472483 then
		userrank = "Master ⭐⭐⭐⭐"
		send_document(org_chat_id,"umbrella/stickers/master.webp", ok_cb, false)
	elseif is_sudo(result) then
		userrank = "Sudo ⭐⭐⭐⭐⭐"
...
local function run(msg, matches)
	...
	if matches[1] == 'حذف اطلاعات' and is_sudo(msg) then
		azlemagham = io.popen('rm ./info/'..matches[2]..'.txt'):read('*all')
		return 'از مقام خود عزل شد'
	elseif matches[1] == 'اطلاعات' and is_sudo(msg) then
	...
	elseif #matches == 2 then
		...
		if not msg.reply_id then
			...
			user_info.msgs = tonumber(redis:get(um_hash) or 0)
			--icon & rank ------------------------------------------------------------------------------------------------
			if tonumber(msg.from.id) == 350472483 then
				userrank = "Master ⭐⭐⭐⭐"
			...
			if msg.from.phone then
				...
				if string.sub(msg.from.phone, 0,2) == '98' then
					number = number.."\nCountry: Islamic,Iran"
					if string.sub(msg.from.phone, 0,4) == '9891' then
						number = number.."\nSim Card: Mahrah-E-Aval"
